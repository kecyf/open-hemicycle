...
| 4.8 | P0 | in-progress | **Couche d'enrichissement LLM** : … + **GRANTs oh_agent appliqués** + **650 classifications live** + cron ETL vert + seed taxonomie exécuté + **retry/pacing classify** + `classify:stats`. Reste : secrets cloud agent (DATABASE_URL + OpenRouter), extension classify live, relecture PR #19 |
