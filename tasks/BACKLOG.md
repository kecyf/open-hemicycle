...
| 4.8 | P0 | in-progress | **Couche d'enrichissement LLM** : … + **650 classifications live** + cron ETL vert + **retry/pacing classify** + `classify:stats` + **workflow GH Actions classify** + **backlog classify admin** + **progression classify admin** + **dispatch classify admin** + **historique runs enrichi + auto-refresh** + **delta classify par run (logs GH)**. Reste : secrets cloud agent (DATABASE_URL auth + OpenRouter), runs classify live, relecture PR #19 |
