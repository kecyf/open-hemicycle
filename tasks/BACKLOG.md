...
| 4.8 | P0 | in-progress | **Couche d'enrichissement LLM** : core parse/seuil + prompt v1 + échantillon-or (15) + `validate:enrichissement` + schéma `scrutins_classifications` + migration SQL `0001` + `check:db` + job `classify:scrutins` (dry-run, skip déjà classifiés) + `resolveEffectiveThemeSlug` + lecture atlas web (`themeScrutinCondition`) + tests ETL + CI validate offline + tests vitest enrichissement. Reste : appliquer migration (HITL), clé OpenRouter, credentials cloud DB, run prod |
