...
- [~] Couche d'enrichissement : classification scrutin → thème par LLM — **fondations + lecture atlas web + décompte source + fiche scrutin + liste scrutins filtrée + index thèmes aligné + diagnostic DB admin + readiness enrichissement admin + retry/pacing classify + workflow GH Actions classify + backlog classify admin** livrées ; migration DB + runs live en attente (secrets + HITL dépense API si cron).
