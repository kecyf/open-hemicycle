...
- [~] Couche d'enrichissement : classification scrutin → thème par LLM — **fondations + lecture atlas web + décompte source + fiche scrutin + liste scrutins filtrée + index thèmes aligné + diagnostic DB admin + readiness enrichissement admin** livrées ; migration DB + run live en attente (HITL + secrets).
